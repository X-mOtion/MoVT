...
class QuantizeEMAReset(nn.Module):
    ...
    @torch.no_grad()
    def update_codebook(self, x, code_idx,idx_layer=0):
        ...
        # #print("Don't update codebook")
        #print(self.codebook.shape,l0.shape)
        #print("idx_layer",idx_layer)
        # if idx_layer == 0:
        #     with torch.no_grad():
        #         #print("*********")
        #         self.codebook.copy_(l0)
        prob = code_count / torch.sum(code_count)
        perplexity = torch.exp(-torch.sum(prob * torch.log(prob + 1e-7)))
    ...
    def forward(self, x, return_idx=False, temperature=0.,idx_layer=0):
        N, width, T = x.shape
        ...
        if self.training:
            #print("*****************")
            perplexity = self.update_codebook(x, code_idx,idx_layer=idx_layer)
        else:
